...
3-1) This method returns an approximate value. How could we get an exact answer ?
...
class RunCollection {
    ...
    public int personalBest() {
        ...
        return runs.stream().filter(r->r.complete).mapToInt(Run::getRunTime).min().orElse(Integer.MAX_VALUE);
    }

    public int bestOfBest() {
        int best = 0;
        for(int c = 0;c<course.obstacleCount;c++){
            int min = Integer.MAX_VALUE;
            for(Run run : runs){
                if(run.obstacleTimes.size()>c){
                    min = Math.min(min,run.obstacleTimes.get(c));
                }
            }
