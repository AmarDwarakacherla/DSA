...
class OrderManager {
    ...
    public Map<Integer, Double> getAverageDeliveryTimeByRestaurant() {
        ...
        for(Order ord : orders){
            for(Delivery del : ord.deliveries){
                time.put(ord.restaurantId,time.getOrDefault(ord.restaurantId,0)+del.getDurationMinutes());
                count.put(ord.restaurantId,count.getOrDefault(ord.restaurantId,0)+1);
            }
        }
        for(Integer t : time.keySet()){
            result.put(t,(double)time.get(t)/count.get(t));
        }
        return result;
    }
