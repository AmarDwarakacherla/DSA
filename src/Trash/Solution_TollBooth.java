...
highway is a divided highway with limited access; the only way on to or
off of the highway is through a toll booth.
There are three types of toll booths:
* ENTRY (E in the diagram) toll booths, where a car goes through a booth
as it enters the highway.
* EXIT (X in the diagram) toll booths, where a car goes through a booth as
it exits the highway.
* MAINROAD (M in the diagram), which have sensors that record a license
plate as a car drives through at full speed.
...
For our first task:
1-1) Read through and understand the code and comments below. Feel free to
run the code and tests.
1-2) The tests are not passing due to a bug in the code. Make the
necessary changes to TollBooth.LogEntry to fix the bug.
*/
/*
We are interested in how many people are using the highway, and so we
would like to count how many complete journeys are taken in the log file.
A complete journey consists of:
* A driver entering the highway through an ENTRY toll booth.
* The driver passing through some number of MAINROAD toll booths (possibly
0).
* The driver exiting the highway through an EXIT toll booth.
For example, the following excerpt of log lines contains complete journeys
for the cars with JOX304 and THX138:
.
.
.
...
→ This log contains 3 complete journeys:
 • JOX304: 1 journey
 • THX138: 2 journeys
You may assume that the log only contains complete journeys, and there are
no missing entries.
2-1) Write a function in TollBooth.LogFile named countJourneys() that returns how
many
complete journeys there are in the given TollBooth.LogFile.
*/
/*
We would like to catch people who are driving at unsafe speeds on the
highway. To help us do that, we would like to identify journeys where a
driver does either of the following:
...
In this case, the driver of TST002 drove 10 km in 275 seconds. We can
calculate
...
------------------- = 130.91 km/hr
 275 sec
Note that:
* A license plate may have multiple journeys in one file, and if they
drive at unsafe speeds in both journeys, both should be counted.
* We do not mark speeding if they are not on the highway (i.e. for any
driving between an EXIT and ENTRY event).
* Speeding is only marked once per journey. For example, if there are 4
segments 120km/h or greater, or multiple segments 130km/h or greater, the
journey is only counted once.
3-1) Write a function catchSpeeders in TollBooth.LogFile that returns a collection
of license plates that drove at unsafe speeds during a journey in the
TollBooth.LogFile.
If the same license plate drives at unsafe speeds during two
different journeys, the license plate should appear twice (once for each
...
class LogFile {
    ...
    */
    ...
    public int countJourneys() {
        int count = 0;
        Map<String, Boolean> activeJourney = new HashMap<>();
        for (LogEntry entry : logEntries) {
            String plate = entry.getLicensePlate();
            if (entry.getBoothType().equals("ENTRY")) {
                activeJourney.put(plate, true);
            } else if (entry.getBoothType().equals("EXIT")) {
                if (activeJourney.containsKey(plate) && activeJourney.get(plate)) {
                    count++;
                    activeJourney.put(plate, false);
                }
//                if (activeJourney.getOrDefault(plate, false)) {
//                    count++;
//                    activeJourney.put(plate, false);
//                }
            }
    ...
    public List<String> catchSpeeders() {
        ...
        Map<String, List<LogEntry>> vehicleLogs = new HashMap<>();
//        for (LogEntry entry : logEntries) {
//            vehicleLogs.computeIfAbsent(entry.getLicensePlate(), k -> new ArrayList<>()).add(entry);
//        }
        for (LogEntry entry : logEntries) {
            String plate = entry.getLicensePlate();
            if (vehicleLogs.containsKey(plate)) {
                vehicleLogs.get(plate).add(entry);
            } else {
                List<LogEntry> list = new ArrayList<>();
                list.add(entry);
                vehicleLogs.put(plate, list);
            }
        }
        for (String plate : vehicleLogs.keySet()) {
            List<LogEntry> entries = vehicleLogs.get(plate);
            boolean inJourney = false;
            boolean speeding = false;
            int highSpeedSegments = 0;
            ...
            for (LogEntry entry : entries) {
                if (entry.getBoothType().equals("ENTRY")) {
                    ...
                    if (entry.getBoothType().equals("MAINROAD") || entry.getBoothType().equals("EXIT")) {
                        float seconds =   Float.parseFloat(entry.getTimestamp())
                                - Float.parseFloat(previous.getTimestamp());
                        float speed = (10 * 3600) / seconds;
                        if (speed >= 120) {
                            highSpeedSegments++;
                        }
                        if (speed >= 130 || highSpeedSegments >= 2) {
                            speeding = true;
                        }
                        previous = entry;
                    }
...
public class Solution_TollBooth {
    ...
    public static void testLogEntry() {
        ...
        //updated this line
        assertEquals("44776.619", logEntry.getTimestamp());
//        assertEquals(44776.619f, logEntry.getTimestamp(), 0.0001);
        ...
        //updated
        assertEquals("52160.132", logEntry.getTimestamp());
